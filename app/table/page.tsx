...
// import {Arrow} from './components/comp1';
import {Button} from '@basic/Button';
import {Cell, Column, Row, TableHeader} from '@basic/Table';
...
import {ComboBox, ComboBoxItem} from '@basic/ComboBox';
import {DatePicker} from '@basic/DatePicker';
import {focusRing} from '@/components/utils';
import {getLocalTimeZone, today} from '@internationalized/date';
import {GridList, GridListItem} from '@basic/GridList';
...
import {Select, SelectItem} from '@basic/Select';
import {Tag, TagGroup} from '@basic/TagGroup';
...
export default function ExampleApp() {
  ...
  const [allItems, setAllItems] = useState(() => plants.map(p => ({...p, isFavorite: false})));
  const [search, setSearch] = useState('');
  const [favorite, setFavorite] = useState(false);
  const [cycles, setCycles] = useState<Selection>(new Set());
  const [sunlight, setSunlight] = useState<Selection>(new Set());
  const [watering, setWatering] = useState<Selection>(new Set());
  ...
  const [visibleColumns, setVisibleColumns] = useState<Selection>(new Set(['favorite', 'common_name', 'sunlight', 'watering', 'actions']));
  const columns = useMemo(() => {
    const res = allColumns.filter(c => visibleColumns === 'all' || visibleColumns.has(c.id!));
  ...
  const addItem = (item: Plant) => {
    setAllItems(allItems => [...allItems, item]);
  };

  const editItem = (item: Plant) => {
    setAllItems(allItems => {
      const items = [...allItems];
  ...
  const deleteItem = () => {
    setAllItems(allItems => {
      ...
      const items = [...allItems];
      const index = items.findIndex(item => item.id === actionItem!.id);
  ...
  const onScroll = (e: UIEvent<HTMLDivElement>) => {
    if (hideOnScroll) {
      const newIsScrolled = e.currentTarget.scrollTop > 0 || e.currentTarget.scrollLeft > 0;
  ...
  const hasAtLeast4Items = items.length >= 4;
  useEffect(() => {
    if (hideOnScroll) {
  ...
  const [dialog, setDialog] = useState<Key | null>(null);
  const [actionItem, setActionItem] = useState<Plant | null>(null);
  const onAction = (item: typeof items[0], action: Key) => {
    switch (action) {
      case 'favorite':
  ...
  return (
    <div className="h-full flex flex-col gap-4 p-4" >
      ...
      <div className="grid grid-cols-[1fr_auto_auto] sm:grid-cols-[1.1fr_auto_auto_1fr_auto] gap-2 items-end">
        <SearchField aria-label="Search" value={search} onChange={setSearch} className="col-span-3 sm:col-span-1" defaultValue={"placeholder=\"Search...\""} />
        <DialogTrigger>
          <TooltipTrigger>
...
function PlantDialog({item, onSave}: {item?: Plant | null, onSave: (item: Plant) => void}) {
  const [droppedImage, setDroppedImage] = useState(item?.default_image?.thumbnail);
  return (
