...
export default function Page() {
  return (
    <article>
      ...
      <Image src={img1} alt="1" className="rounded-xl shadow-2xl mx-auto"/>
      <p>在电脑端，可以使用开发者调试工具获取用户的用户名<Code>username</Code>，这一字段可以真正地定位用户对象。</p>

      查询用户基本信息的一个接口后台返回的数据格式如下：
      <pre className="p-3 bg-slate-800 rounded-2xl mx-1 md:mx-5 text-white overflow-auto">
        {code}
      </pre>
      <p>这个接口的链接是 <i className="text-default-500 text-wrap">tieba.baidu.com/i/sys/user_json?un=[username]&ie=utf-8</i>，借助这个接口可以定位<Code>id</Code>，
      ...
      <Image src={img2} alt="1" className="rounded-xl shadow-2xl mx-auto"/>
    </article>
  )
};